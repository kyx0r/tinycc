...
#define CONFIG_TCC_CRTPREFIX USE_TRIPLET(CONFIG_SYSROOT "/usr/lib")
#endif
/* Below: {B} is substituted by CONFIG_TCCDIR (rsp. -B option) */
/* system include paths */
#ifndef CONFIG_TCC_SYSINCLUDEPATHS
#define CONFIG_TCC_SYSINCLUDEPATHS "{B}/include" PATHSEP ALSO_TRIPLET(CONFIG_SYSROOT "/usr/include")
#endif
/* library search paths */
#ifndef CONFIG_TCC_LIBPATHS
#define CONFIG_TCC_LIBPATHS "{B}" PATHSEP ALSO_TRIPLET(CONFIG_SYSROOT "/usr/lib")
#endif
...
/* eventually made SHF_WRITE in sort_sections() */
...
static ElfW(Phdr) *update_phdr(ElfW(Phdr) *ph, int type, Section *s,
			       addr_t addr, int file_offset)
{
	if (ph->p_type == 0) {
		fill_phdr(ph, type, s);
	}
...
{
	int size, align, reg_count;
	if (classify_x86_64_arg(vt, ret, &size, &align,
				&reg_count) == x86_64_mode_memory)
		return 0;
